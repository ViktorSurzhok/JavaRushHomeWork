...
public class Solution {
	public static void main(String[] args) throws IOException {
		...
		ArrayList<String> listCase = new ArrayList<String>();
		for (int i = 0; i < list.size(); i++) {
			if (list.get(i).length() % 2 == 0) {
				listCase.add(list.get(i) + " " + list.get(i));
			} else {
				listCase.add(list.get(i) + " " + list.get(i) + " " + list.get(i));
			}
		}

		for (int i = 0; i < listCase.size(); i++) {
			System.out.println(listCase.get(i));
		}
	}
}
